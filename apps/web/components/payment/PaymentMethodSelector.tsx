      ...
      <div className="grid grid-cols-2 gap-3 sm:flex">
        {sortedMethods.map((method) => (
          <button
            ...
            onClick={() => method.available && onSelect(method.type)}
          >
            <span className="flex items-center gap-2">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={methodIcon(method.type)} alt={t(`payment.methods.${method.type}`)} className="h-7 w-7 object-contain" />
              <span className="flex flex-col items-start leading-none">
                <span className="text-base font-semibold">{t(`payment.methods.${method.type}`)}</span>
                {method.fee_rate > 0 ? (
                  <span className="text-[10px] tracking-wide text-gray-500 dark:text-dark-400">
