...
interface ReAuthAccountModalProps {
  ...
  onClose: () => void
  onReauthorized: (account: Account) => void
}
...
export default function ReAuthAccountModal({
  ...
  const handleExchangeCode = async () => {
    ...
    if (isOpenAILike) {
      ...
      try {
        const updatedAccount = await adminAccountsAPI.applyOAuthCredentials(account.id, {
          type: 'oauth',
          credentials: openaiOAuth.buildCredentials(tokenInfo),
          extra: openaiOAuth.buildExtraInfo(tokenInfo),
        })
        appStore.showSuccess(t('admin.accounts.reAuthorizedSuccess'))
        onReauthorized(updatedAccount)
        handleClose()
      } catch (error: unknown) {
        appStore.showError(
      ...
      try {
        ...
        const updatedAccount = await adminAccountsAPI.clearError(account.id)
        appStore.showSuccess(t('admin.accounts.reAuthorizedSuccess'))
        onReauthorized(updatedAccount)
        handleClose()
      } catch (error: unknown) {
