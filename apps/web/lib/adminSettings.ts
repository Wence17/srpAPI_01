...
export function buildAuthSourceDefaultsState(
  ...
  return AUTH_SOURCE_TYPES.reduce((acc, source) => {
    ...
    acc[source] = {
      balance: Number(
        raw[`auth_source_default_${source}_balance`] ?? AUTH_SOURCE_DEFAULT_BALANCE,
      ),
      concurrency: Math.max(
        1,
        Number(
          raw[`auth_source_default_${source}_concurrency`] ?? AUTH_SOURCE_DEFAULT_CONCURRENCY,
        ),
      ),
...
export function appendAuthSourceDefaultsToUpdateRequest(
  ...
  for (const source of AUTH_SOURCE_TYPES) {
    ...
    target[`auth_source_default_${source}_platform_quotas`] = sanitizePlatformQuotasMap(
      current.platform_quotas,
    )
  }
...
/** Backward-compatible alias used by the Next.js store and settings page. */
...
/** Backward-compatible alias used by the Next.js settings page. */
export type UpdateAdminSettingsRequest = UpdateSettingsRequest
