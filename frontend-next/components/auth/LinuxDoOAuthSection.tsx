...
interface LinuxDoOAuthSectionProps {
  disabled?: boolean
  affCode?: string
  showDivider?: boolean
}

export default function LinuxDoOAuthSection({
  disabled = false,
  affCode,
  showDivider = true,
}: LinuxDoOAuthSectionProps) {
  const { t } = useI18n()
  ...
  function startLogin(): void {
    ...
    const startURL = `${getApiBaseUrl()}/auth/oauth/linuxdo/start?redirect=${encodeURIComponent(redirectTo)}`
    ...
    <div className="space-y-4">
      <button type="button" disabled={disabled} className="btn btn-secondary w-full" onClick={startLogin}>
        <svg
          ...
          width="1em"
          height="1em"
          style={{ color: 'rgb(233, 84, 32)', width: 20, height: 20 }}
          aria-hidden="true"
        >
